...
<article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
    ...
    @if($beritaTerkait->count() > 0)
        <section>
            <h2 class="text-xl font-bold text-gray-900 mb-6">Berita Lainnya</h2>
            <div class="grid md:grid-cols-3 gap-6">
                @foreach($beritaTerkait as $item)
                    <article class="bento-card group">
                        @if($item->gambar)
                            <img src="{{ $item->gambar_url }}" alt="{{ $item->judul }}" class="w-full h-32 object-cover rounded-lg mb-3">
                        @else
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{ $item->tanggal_publikasi?->format('d M Y') }}</p>
                    </article>
                @endforeach
            </div>
        </section>
