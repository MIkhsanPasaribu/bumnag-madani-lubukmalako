...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900">Edit Laporan Keuangan</h2>
            <p class="text-sm text-gray-500">{{ $unit->nama }} — {{ $laporan->periode }}</p>
        </div>

        <form action="{{ route('unit.laporan-keuangan.update', $laporan) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="bulan" id="bulan" class="form-input w-full rounded-lg border-gray-300" required>
                        @foreach(\App\Models\LaporanKeuangan::$namaBulan as $key => $nama)
                            <option value="{{ $key }}" {{ old('bulan', $laporan->bulan) == $key ? 'selected' : '' }}>{{ $nama }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="number" name="pendapatan" id="pendapatan" value="{{ old('pendapatan', $laporan->pendapatan) }}"
            ...
            <div>
                ...
                <input type="number" name="pengeluaran" id="pengeluaran" value="{{ old('pengeluaran', $laporan->pengeluaran) }}"
                       min="0" step="1" class="form-input w-full rounded-lg border-gray-300" required>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                          class="form-input w-full rounded-lg border-gray-300">{{ old('keterangan', $laporan->keterangan) }}</textarea>
                @error('keterangan') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="btn-primary px-6 py-2.5">Simpan Perubahan</button>
